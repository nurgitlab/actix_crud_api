...
pub async fn apply_migrations(pool: &PgPool) -> Result<(), Box<dyn Error>> {
    // Создаем таблицу для отслеживания миграций вне транзакции
    // (CREATE TABLE IF NOT EXISTS обычно не требует транзакции)
    sqlx::query(
    ...
    for migration_dir in migration_dirs {
        ...
        if let Some(version) = version {
            if !applied_migrations.contains(&version) {
                ...
                let up_path = migrations_dir.join(&migration_dir).join("up.sql");
                if !up_path.exists() {
